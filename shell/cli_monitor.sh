...
function ai_wrapper() {
    ...
    local _vscode_ipc_hook_cli="$(_ai_meta_sanitize "${VSCODE_IPC_HOOK_CLI:-}")"
    local _vscode_git_askpass_main="$(_ai_meta_sanitize "${VSCODE_GIT_ASKPASS_MAIN:-}")"
    local _vscode_git_askpass_node="$(_ai_meta_sanitize "${VSCODE_GIT_ASKPASS_NODE:-}")"
    local _vscode_git_ipc_handle="$(_ai_meta_sanitize "${VSCODE_GIT_IPC_HANDLE:-}")"
    local _vscode_injection="$(_ai_meta_sanitize "${VSCODE_INJECTION:-}")"
    local _cursor_trace_id="$(_ai_meta_sanitize "${CURSOR_TRACE_ID:-}")"
    local _terminal_emulator="$(_ai_meta_sanitize "${TERMINAL_EMULATOR:-}")"
    local _idea_initial_directory="$(_ai_meta_sanitize "${IDEA_INITIAL_DIRECTORY:-}")"
    local _jetbrains_ide_name="$(_ai_meta_sanitize "${JETBRAINS_IDE_NAME:-}")"
    local _jetbrains_ide_product="$(_ai_meta_sanitize "${JETBRAINS_IDE_PRODUCT:-}")"
    local _android_studio_version="$(_ai_meta_sanitize "${ANDROID_STUDIO_VERSION:-}")"
    ...
    echo "--- MONITOR_META vscode_ipc_hook_cli: ${_vscode_ipc_hook_cli} ---" >> "$log_file"
    echo "--- MONITOR_META vscode_git_askpass_main: ${_vscode_git_askpass_main} ---" >> "$log_file"
    echo "--- MONITOR_META vscode_git_askpass_node: ${_vscode_git_askpass_node} ---" >> "$log_file"
    echo "--- MONITOR_META vscode_git_ipc_handle: ${_vscode_git_ipc_handle} ---" >> "$log_file"
    echo "--- MONITOR_META vscode_injection: ${_vscode_injection} ---" >> "$log_file"
    echo "--- MONITOR_META cursor_trace_id: ${_cursor_trace_id} ---" >> "$log_file"
    echo "--- MONITOR_META terminal_emulator: ${_terminal_emulator} ---" >> "$log_file"
    echo "--- MONITOR_META idea_initial_directory: ${_idea_initial_directory} ---" >> "$log_file"
